...
function getNextFriday(date) {
  ...
  const offset = day >= 5 ? (7 % day) + 5 : (day % 7) + 1;
  const newDate = new Date(copyDate.setDate(copyDate.getDate() + offset));
  return newDate;
}
...
function getCountWeekendsInMonth(month, year) {
  const day = new Date(year, month - 1, 2) - new Date(year, month - 1, 1);
  ...
  } while (date.getMonth() === initialDate.getMonth());
...
function getNextFridayThe13th(date) {
  ...
  const day = new Date(0, 0, 2) - new Date(0, 0, 1);
  while (date) {
    if (copyDate.getDay() === 5 && copyDate.getDate() === 13) break;
    else copyDate = new Date(copyDate.getTime() + day);
  }
  return copyDate;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const [startDay, startMonth, startYear] = period.start
    .split('-')
    .map((item) => +item);
  const [endDay, endMonth, endYear] = period.end
    .split('-')
    .map((item) => +item);

  const start = new Date(startYear, startMonth - 1, startDay).getTime();
  const end = new Date(endYear, endMonth - 1, endDay).getTime();
  let workCount = countWorkDays;
  let offCount = countOffDays;
  let workDay = true;
  ...
  for (let i = start; i <= end; i += day) {
    if (workDay) {
      ...
      schedule.push(`${curDay}-${curMonth}-${curYear}`);
      workCount -= 1;
      if (!workCount) {
        workDay = false;
        workCount = countWorkDays;
      }
    } else {
      offCount -= 1;
      if (!offCount) {
        workDay = true;
        offCount = countOffDays;
      }
    }
  }
  return schedule;
